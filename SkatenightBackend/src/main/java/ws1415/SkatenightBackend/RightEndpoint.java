...
public class RightEndpoint extends SkatenightServerEndpoint{
    ...
    public void distributeRightToUser(User user, @Named("groupName") String groupName,@Named("rightName") String rightName, @Named("userName") String userName)throws  OAuthRequestException{
        ...
        if(hasRights(group, user.getEmail(), Right.DISTRIBUTERIGHTS.name())){
            ...
            if(!group.getMemberRights().get(userName).contains(rightName)){
     ...
     * @throws IllegalArgumentException
     */
    public void distributeRightsToUser(User user, @Named("groupName") String groupName, @Named("rightNames") List<String> rightNames, @Named("userName") String userName)throws  OAuthRequestException{
        EndpointUtil.throwIfNoUser(user);
        EndpointUtil.throwIfUserGroupNameWasntSubmitted(groupName);
        UserGroup group = EndpointUtil.throwIfNoUserGroupExists(groupName);
        if(hasRights(group, user.getEmail(), Right.DISTRIBUTERIGHTS.name())){
            ...
            for(String right : rightNames){
                if(!group.getMemberRights().get(userName).contains(right)){
     ...
     * @throws IllegalArgumentException
     */
    public void distributeRightToUsers(User user, @Named("groupName") String groupName, @Named("rightName") String rightName, @Named("userNames") List<String> userNames)throws  OAuthRequestException{
        EndpointUtil.throwIfNoUser(user);
        EndpointUtil.throwIfUserGroupNameWasntSubmitted(groupName);
        UserGroup group = EndpointUtil.throwIfNoUserGroupExists(groupName);
        if(hasRights(group, user.getEmail(), Right.DISTRIBUTERIGHTS.name())){
            for(String member : userNames){
                ...
                if(!group.getMemberRights().get(member).contains(rightName)){
                    group.getMemberRights().get(member).add(rightName);
                }
            }
            ofy().save().entity(group).now();
        } else{
     ...
     * @throws IllegalArgumentException
     */
    public void distributeRightsToUsers(User user, @Named("groupName") String groupName, @Named("rightNames") List<String> rightNames, @Named("userNames") List<String> userNames)throws  OAuthRequestException{
        EndpointUtil.throwIfNoUser(user);
        EndpointUtil.throwIfUserGroupNameWasntSubmitted(groupName);
        UserGroup group = EndpointUtil.throwIfNoUserGroupExists(groupName);
        if(hasRights(group, user.getEmail(), Right.DISTRIBUTERIGHTS.name())){
            for(String member : userNames){
                EndpointUtil.throwIfUserNotInGroup(group, member);
                for(String right : rightNames){
                    if(!group.getMemberRights().get(member).contains(right)){
